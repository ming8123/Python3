...
def is_valid(dni):
    ...
    return True
if __name__ == "__main__":#lo que hace es que puedes ejecutar funcion de prueba
    import doctest
    doctest.testmod()
...
def edit():
    ...
    for i, client in enumerate(clients):
        if client['dni'] == dni:
            ...
            clients[i]['nombre'] = help.input_text(2, 30)
            print(f"Introduce nuevo apellido ({client['apellido']})")
            clients[i]['apellido'] = help.input_text(2, 30)
            return True
    return False
